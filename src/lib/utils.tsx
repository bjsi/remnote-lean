...
export function regexMap<T>(
  regex: RegExp,
  s: string,
  f_no_match: (snm: string) => T,
  f_match: (m: RegExpExecArray) => T
): T[] {
...
export function colorizeMessage(goal: string, darkMode: boolean): JSX.Element {
  // `replace(r, match)(cont)(s)` replaces `r` with `match` in `s`, and runs `cont` on
  // non-matches.
  const replace =
    <T,>(r: RegExp, match: (s: string) => T) =>
    (cont: (s: string) => T[]) =>
    (s: string): T[] => {
  ...
  const pipeline = [
    replace(/^([|⊢]) /gm, (x) => (
      <strong
        className="goal-vdash"
        style={{
          color: darkMode ? '#569cd6' : '#367cb6',
        }}
      >
        {x}&nbsp;
      </strong>
    )),
    replace(/^(\d+ goals|1 goal)/gm, (x) => (
      <strong
        className="goal-goals"
        style={{
          color: darkMode ? '#569cd6' : '#367cb6',
        }}
      >
